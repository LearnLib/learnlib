 ...
 *         a function for combining simultaneously occurring outputs of timers
 * @param maxTimeoutWaitingTime
 *         the maximum time to wait for a timeout in any configuration. If no timeout is observed after this time, the
 ...
 *         the maximum waiting time to wait when inferring timers for a location. This must be at least the maximum time
 *         for a timeout. We recommend setting this value to at least twice the highest value of any timer in the SUL,
 *         if these values are known or can be estimated. This increases the likelihood of detecting non-periodic
 *         behavior during timer inference, and thus reduces the need for equivalence queries.
 * @param <O>
 *         output symbol type
 */
