...
public class TTTLearnerDFAASTTest extends AbstractLearnerASTDFATest<TTTLearnerDFA<Character>> {

    @Override
    protected TTTLearnerDFA<Character> getLearner(DFAMembershipOracle<Character> oracle, Alphabet<Character> alphabet) {
        return new TTTLearnerDFA<>(alphabet, oracle);
    }

    @Override
    protected Collection<Word<Character>> getTrueRepresentatives() {
        final BaseTTTDiscriminationTree<Character, Boolean> dt = learner.getDiscriminationTree();
