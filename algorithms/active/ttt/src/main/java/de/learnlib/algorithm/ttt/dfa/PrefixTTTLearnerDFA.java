...
public class PrefixTTTLearnerDFA<I> extends TTTLearnerDFA<I> {
    ...
    protected boolean refineHypothesisSingle(DefaultQuery<I, Boolean> ceQuery) {
        ...
        do {
            ...
            final int breakpoint = analyzer.analyzeAbstractCounterexample(acex, 0, currReachInconsLength);
            final ExtDTNode<I> toSplit = acex.getHypNode(breakpoint);
            final TTTState<I, Boolean> splitState = toSplit.getData();
            final ExtDTNode<I> lca = acex.getLCA(breakpoint + 1);
            final I sym = ceWord.getSymbol(breakpoint);
            final Word<I> newDiscr = lca.getDiscriminator().prepend(sym);
            final ExtDTNode<I> succHyp = acex.getHypNode(breakpoint + 1);
            final Boolean hypOut = lca.subtreeLabel(succHyp);
            assert hypOut != null;
            openTransitions.concat(toSplit.getIncoming());
            final ExtDTNode<I>.SplitResult splitResult = toSplit.split(newDiscr, hypOut, !hypOut);
            link(splitResult.nodeOld, splitState);
            final ExtDTNode<I> extUnlabeled = (ExtDTNode<I>) splitResult.nodeNew;
