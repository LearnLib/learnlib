...
/**
 * A variant of the backtracking ADS search (see {@link net.automatalib.util.automata.ads.ADS},
 * {@link net.automatalib.util.automata.ads.BacktrackingSearch}), that works on partially defined automata. It tries to
 * find an ADS based on defined transitions and successively closes transitions if no ADS has been found.
 *
...
public class DefensiveADS<S, I, O> {
	 ...
	 * @param pta       the analyzer to inspect and close partial transitions
	 * @param <S>       (hypothesis) state type
	 * @param <I>       input alphabet type
	 * @param <O>       output alphabet type
	...
	private Optional<ADTNode<S, I, O>> compute(final Map<S, S> mapping) {
		...
		while (!splittingWordCandidates.isEmpty()) {
			...
			final Map<S, S> currentToInitialMapping = mapping.keySet()
					.stream()
					.collect(Collectors.toMap(x -> automaton.getSuccessor(x, prefix), mapping::get));
