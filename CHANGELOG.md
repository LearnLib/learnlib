...
* The `BasePTA` now is an automaton over an input type `I` and reads its traces as `Word<I>` to provide a more convenient integration into the existing types of AutomataLib/LearnLib. Users who are concerned with performance, may use an `IntRange` or `FastAlphabet` to replicate previous `int[]` characteristics.
