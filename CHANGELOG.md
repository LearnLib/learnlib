...
* The JPMS support introduces several changes:
  ...
  * Split packages had to be refactored.
    ...
    * The `OTUtils` class no longer provides the `displayHTMLInBrowser` methods in order to not depend on `java.desktop`. If you relied on this functionality, use the `writeHTMLToFile` methods instead and call `Desktop.getDesktop().open(file)` yourself.
