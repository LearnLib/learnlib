...
public final class ReuseTree<S, I, O> implements Graph<ReuseNode<S, I, O>, ReuseEdge<S, I, O>> {
    ...
    /**
     * Returns a reuseable {@link NodeResult} with system state or {@code null} if none such exists. If
    ...
    /**
     * Inserts the given {@link Word} with {@link QueryResult} into the tree starting from the root node of the tree.
     * For the longest known prefix of the given {@link Word} there will be no new nodes or edges created.
     * <p>
     * Will be called from the {@link ReuseOracle} if no system state was available for reusage for the query (otherwise
     * {@link #insert(Word, ReuseNode, QueryResult)} would be called). The last node reached by the last symbol of the
     * query will hold the system state from the given {@link QueryResult}.
     * <p>
    ...
    /**
     * Inserts the given {@link Word} (suffix of a membership query) with {@link QueryResult} (suffix output) into the
     * tree starting from the {@link ReuseNode} (contains prefix with prefix output) in the tree. For the longest known
     * prefix of the suffix from the given {@link Word} there will be no new nodes or edges created.
     * <p>
     * Will be called from the {@link ReuseOracle} if an available system state was reused for the query (otherwise
     * {@link #insert(Word, QueryResult)} would be called). The old system state was already removed from the {@link
     * ReuseNode} (through {@link #fetchSystemState(Word)}) if the ''invalidateSystemstates'' flag in the {@link
     * ReuseOracle} was set to {@code true}.
     * <p>
     * This method should only be invoked internally from the {@link ReuseOracle} unless you know exactly what you are
     * doing (you may want to create a predefined reuse tree before start learning).
     *
