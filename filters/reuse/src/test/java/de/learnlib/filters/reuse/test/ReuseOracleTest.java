...
public class ReuseOracleTest {
	...
	protected void setUp() {
		...
		ReuseCapableOracle<Integer, Integer, String> reuseCapableOracle = new ReuseCapableOracle<Integer, Integer, String>() {
			...
			public QueryResult<Integer, String> continueQuery(
					Word<Integer> trace, ReuseNode<Integer, Integer, String> s) {
				return null;
			}
	...
	public void testTreeIsEmpty() {
		ReuseNode<Integer, Integer, String> node = null;
	...
	public void testNoReusePossible() {
		...
		ReuseNode<Integer, Integer, String> node = reuseOracle.getReuseTree().getReuseableSystemState(getInput(1,2));
		Assert.assertNull(node);
	}
	...
	public void testReusePossibleWithInvalidation() {
		...
		Assert.assertTrue(node.getPrefixInput().size() == 2); // query '1 1'
	...
	public void testReusePossibleWithoutInvalidation() {
		...
		reuseOracle.getReuseTree().insert(getInput(4), node, qr);
	...
	public void testReuseNodePrefixWhileReusing() {
		...
		Assert.assertTrue(node.getPrefixInput().size() == 2); // invariant input is trimmed
		// reuse the prefix
		...
		// The output should also be known, but the node contains only the prefix without
		// model invariant outputs
		Word<String> output = getOutput("ok", "ok", "ok");
		Assert.assertEquals(node.getPrefixOutput(), output);
		
		output = reuseOracle.getReuseTree().getOutput(getInput(1,0,0,0,0,1,1));
		Assert.assertEquals(output, getOutput("ok", "ok", "ok", "ok", "ok", "ok", "ok"));
	}
